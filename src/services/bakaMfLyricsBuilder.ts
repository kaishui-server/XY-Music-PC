...
function buildWordTimeCandidate(
  ...
  for (const wordTime of wordTimes) {
    ...
    if (previousStart !== null && wordStartMs < previousStart) backwardCount++;
...
function selectWordTimeEntries(
  ...
  const candidates = [relativeCandidate, kuwoCandidate]
    .filter((candidate): candidate is WordTimeCandidate => candidate !== null);
  if (lineHasNegativeWordTime) candidates.sort((left, right) => left.score - right.score);
  else candidates.sort((left, right) => left.score - right.score);
